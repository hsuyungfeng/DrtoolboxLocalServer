...
- **AI 預擬草案 (Drafting Fallback)**：當本地資料庫數據不足時，系統會自動切換至「AI 預擬模式」，產出標註為 **`[AI 預擬草案]`** 的專業回答，確保醫師永遠有高品質的基礎草案可以進行補充校正。
- **AI 校正編輯器**：支援 **「✨ AI 重新潤飾」**，可一鍵啟動聯網搜尋模式，彙整台灣最新醫學建議。

### 4. 🧠 診所知識地圖與全域推理
- **視覺化知識圖譜**：Dashboard 新增「診所知識地圖」標籤，透過 D3.js 呈現 AI 腦中的邏輯關聯，點擊節點即可查看相關文獻。
- **跨文件邏輯鏈 (Global Reasoning)**：系統每晚自動執行「全域掃描」，分析不同文件間的協同或矛盾，並自動產出全域臨床洞察。
- **深度影像分析**：上傳術前術後照片時，AI 會自動執行「臨床影像分析」，產出包含紅腫、結痂、修復階段的結構化進度筆記。

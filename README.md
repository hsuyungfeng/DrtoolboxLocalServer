...
- **SOAP 語音轉錄與雙 LLM 擬真對比實驗室**：整合 SoapVoice 語音引擎與門診畫面 (DoctorConsultation) OCR 解析，支援一鍵從門診截圖提取病患姓名、生日與病歷號，並並行執行「雲端 Baseline (無 DB)」與「Local LLM (Ornith-1.0-9B + DB)」的擬真 SOAP 病歷生成與醫師盲測投票。

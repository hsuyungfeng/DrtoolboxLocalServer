#!/usr/bin/env python3
"""
Generate batch graph outputs for ALL 14 batches using extract results.
"""
...
def generate_tags(path, language, file_category, extract_result):
    ...
    functions = extract_result.get("functions", []) if extract_result else []
    classes = extract_result.get("classes", []) if extract_result else []
    endpoints = extract_result.get("endpoints", []) if extract_result else []
...
def build_nodes_and_edges(batch_index, batch_files, extract_results, import_data, neighbor_map):
    ...
    # Import edges from batchImportData (cross-batch edges)
    seen_edges = set()
    ...
    # For Python files, scan imports against all files in this batch
    for file_info in batch_files:
        p = file_info["path"]

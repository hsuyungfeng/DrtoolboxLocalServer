...
def process_and_load_directory(directory):
    """Processes any raw files to .txt, then loads all .txt files."""
    documents = []
        ...
        for file in files:
            ...
            if ext in supported_exts:
                ...
                if not os.path.exists(txt_path):
                    logger.info(f"Extracting text from new file: {filepath}")
...
def get_special_data():
    ...
    docs = process_and_load_directory(SPECIAL_DATA_DIR)
    ...
    if PREFERRED_SPECIAL_DIR != SPECIAL_DATA_DIR:
        try:
            if os.path.exists(PREFERRED_SPECIAL_DIR):
                ...
                external_docs = process_and_load_directory(PREFERRED_SPECIAL_DIR)
...
def get_general_data():
    ...
    docs = process_and_load_directory(GENERAL_DATA_DIR)
    ...
    if PREFERRED_GENERAL_DIR != GENERAL_DATA_DIR:
        try:
            if os.path.exists(PREFERRED_GENERAL_DIR):
                ...
                external_docs = process_and_load_directory(PREFERRED_GENERAL_DIR)
                docs.extend(external_docs)
        except Exception as e:
